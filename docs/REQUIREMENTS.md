...
- 登录页打开后用 `code` 换 openid 认人。库里有真实昵称才显示「欢迎回来」并带出头像昵称；openid 不存在，或昵称仍是空/「微信用户」，走完善资料。删掉的是别的成员时，当前微信号只要还在 `user` 表里就仍是老用户。主动退出后不自动静默登录。
- 头像是本地临时文件，须登录拿到 token 后再上传；不要把临时路径写入数据库。登录请求不带资料时不得覆盖库里已有头像昵称；「微信用户」和微信默认灰头像也视为无效，不得覆盖。
...
- 普通成员：浏览上架菜品、购物车、下单、查看自己点的单和自己做的单、接单/拒绝/完成（若被选为制作人）、待接单时取消自己的单。
...
状态：`-2` 制作人已拒绝，`-1` 已取消，`0` 待接单，`1` 制作中，`2` 已完成。

- 「我的点单」按 `customer_uuid` 过滤（我下的单）；「我的制作」按 `maker_uuid` 过滤（指定我做的单）。
